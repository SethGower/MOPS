...
static Status add_logentry(int line, int id, int date, int time, int loc,
                           uint8_t code) {
...
static void print_date(int date, FILE *fp) {
    fprintf(fp, "%4d/%02d/%02d", date / 10000, (date / 100) % 100, date % 100);
}
...
int load_logfile(FILE *file) {
    ...
    while (getline(&buf, &len, file) >= 0) {
        ...
        if (num != 5) {
            fprintf(stderr, "load_logfile(): line %d has %d fields", nlines,
                    num);
            fputs("; ignoring line\n", stderr);
            continue;
        }
        ...
        Status result;
        result = add_logentry(nlines, (int)strtol(fields[0], NULL, 10),
    ...
    for (entry = logfile; entry; entry = entry->next) {
        ...
        if (emp == NULL) {
            ...
            for (event = entry->first; event; event = event->next) {
                ...
                printf(" %02d:%02d ", event->time / 100, event->time % 100);
